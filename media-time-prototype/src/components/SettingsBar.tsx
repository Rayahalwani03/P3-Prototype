...
export function SettingsBar() {
  ...
  return (
    <div className="fixed inset-x-0 top-4 z-40 flex justify-end px-4">
      <div className="flex items-center gap-4 rounded-full border border-neutral-200 bg-white/80 px-4 py-2 text-xs shadow-soft backdrop-blur dark:border-neutral-700 dark:bg-neutral-900/70">
        <button
          type="button"
          onClick={handleExit}
          disabled={!isSessionActive}
          className="rounded-full border border-rose-200 bg-rose-50 px-3 py-1 font-semibold text-rose-700 transition hover:bg-rose-100 disabled:cursor-not-allowed disabled:border-neutral-300 disabled:bg-neutral-200 disabled:text-neutral-500 dark:border-rose-500/40 dark:bg-rose-500/15 dark:text-rose-200 dark:hover:bg-rose-500/25 dark:disabled:border-neutral-700 dark:disabled:bg-neutral-800 dark:disabled:text-neutral-500"
        >
          {messages.settings.exitLabel}
        </button>
        <div className="flex items-center gap-2">
          <span className="font-semibold uppercase tracking-[0.3em] text-neutral-500 dark:text-neutral-400">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            onClick={toggleTheme}
            className="rounded-full border border-brand-200 bg-white px-3 py-1 font-semibold text-brand-700 shadow-sm transition hover:bg-brand-50 dark:border-neutral-600 dark:bg-neutral-800 dark:text-neutral-100 dark:hover:bg-neutral-700"
          >
